...
class _NiraHomePageState extends State<NiraHomePage> {
  ...
  Future<void> _sendMessage() async {
    ...
    setState(() {
      ...
      _messages.add(ChatMessage(text: 'NIRA is thinking...', isUser: false));
    ...
    try {
      ...
      ProcessingStrategy.selectMethod(profile.level, intent);
      ...
      switch (intent) {
        ...
        case IntentType.calculator:
          ...
          if (result == null) {
            response = 'I could not calculate that expression.';
          } else {
            response = CalculatorService.formatResult(result);
          }
          break;
        ...
        case IntentType.general:
          response = await GemmaService.generateResponse(text);
          break;
      }
              ...
              child: Row(
                children: [
                  ...
                  Icon(
                    ...
                    color: _batteryLevel <= 20 ? Colors.red : Colors.grey,
                  ),
  ...
  void _showSettings() {
    Navigator.push(
      ...
      MaterialPageRoute(builder: (context) => const NiraSettingsPage()),
    );
  }
}
...
class _SettingsSectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Icon(icon, size: 21, color: Colors.indigo),
